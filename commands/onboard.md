...
2. **Contract** — run `bash ${CLAUDE_PLUGIN_ROOT}/scripts/contract-drift.sh .` and show the human the report in full, including its `NOT EXAMINED` tail. Do not compress it into a verdict, and never report the repo "current": the previous version of this step tested only the State honesty block, passed all three onboarded repos, and left every one of them carrying pre-BD-25 Focus wording while hooks enforced the new rules (BD-40, BD-34). Then walk the report block by block with the human: which `MISSING` template lines to add, and which `REPO-ONLY` lines are deliberate adaptation to keep. Apply only what they confirm. Never bulk-replace the section with the template (`${CLAUDE_PLUGIN_ROOT}/templates/claude-md-section.md`) — every live repo carries adaptations a flattening refresh would silently delete, and the script cannot tell those from stale wording, which is why it asks instead of deciding.
