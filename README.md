...
- **Guards are string-matching**, not a shell parser. Bash-side writes to task/plan files (`sed -i`, redirects, `tee`, `mv`/`cp`/`rm`) are caught best-effort; a determined bypass is still possible. Good enough for honest-but-forgetful; not adversarial-proof. Three known false positives, all failing safe: a gated status name inside quoted prose on a `task edit` (e.g. a note quoting the flag and the word Done), a single command line that mixes a read-only status filter with an unrelated edit, and one command segment carrying both a task creation and an unrelated `-p` belonging to another tool. Rephrase the note, or split the line. Read-only queries on their own (`task list -s <status>`, `board -s <status>`) are exempt.
